...
6. `GATEWAY_REQUIRE_AUTH` (default: `false`)
...
3. If `GATEWAY_REQUIRE_AUTH=true`, missing bearer token is rejected with `401`.
4. If `GATEWAY_REQUIRE_AUTH=false`, `tenant_id` from payload is used as local/dev fallback.
...
5. If `GATEWAY_REQUIRE_AUTH=true`, WebSocket connect requires a valid bearer token.
